 ...
 * @param {String} task.ts the timestamp of the request that is coming
...
function check_if_year_db(task,cb){
    ...
    // every time
    if(cdb_year_result !== undefined){
        if( cdb_year_result[1] !== year_result[1] ){
            // year is already in db name, but not the same year, so croak
            throw new Error('year mismatch.  db is set as '+db+' but checking for '+year_result[1])
        }else{
    ...
    var year_couchdb = 'http://'+cdb+'/'+db+'%2f'+year_result[1]
    ...
    // bad
    superagent.put(year_couchdb).type('json')
        .end(function(err,result){
            ...
            task.options.couchdb.db = db +'%2f'+year_result
            cb(null,task)
            return null
        })
    return null
