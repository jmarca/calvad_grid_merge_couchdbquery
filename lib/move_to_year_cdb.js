 ...
 * @returns {}
 */
function get_doc_count (config,cb){

    var cdb = [config.couchdb.host+':'+config.couchdb.port
 ...
 * bulkdocs to delete teh docs, and then one key per year seen.  If
 * the docs are all in one year, then you'll only get one key aside
 * from 'original'
 *
 * @returns {} null
 */
function get_some_docs(config, cb){
