...
function get_hpms_fractions_one_hour(task,cb){
    ...
    var date = task.ts
    var q = queue(5)
    // generate a list of all cell_ids, and append the given hour
    var allkeys = cellids.map(function(id){
        return id + "_"+date
    })
    var fetchkeys
    function fetch_some (keys,cbfetch){
        var opts = _.assign({}
                            ,task.options.couchdb
                            ,{keys:keys
        ...
        getter(opts
               ,function(err,docs){
                   ...
                   if(docs !== undefined
                      && docs.rows !== undefined
                      && docs.rows.length>0){
                       docs.rows = _.filter(docs.rows,function(row){
                       ...
                       _.forEach(docs.rows,function(row){
                           hpms_fractions[row.doc.geom_id]={}
                           cells++
                           if(row.doc[aadt_element])
                           _.each(row.doc[aadt_element],function(v,k){
                               if(!v) v = 0 // force null, undefined to be zero
                               hpms_fractions[row.doc.geom_id][k]=+v
                           })
                   })
                   }else{
                   ...
                   hpms_fractions_cells =  cells
                   return cbfetch(null,{"hpms_fractions":hpms_fractions,
                                   "hpms_fractions_cells":hpms_fractions_cells})
               })
    }
    // loop
    while(allkeys.length > 0){
        fetchkeys = allkeys.splice(0,100)
        q.defer(fetch_some, fetchkeys)
    }
    q.awaitAll(function (e,results){
        task.hpms_fractions = {}
        task.hpms_fractions_cells = 0
        results.map(function(v){
            if(v.hpms_fractions === undefined){
                return null
            }
            _.assign(task.hpms_fractions,v.hpms_fractions)
            task.hpms_fractions_cells += v.hpms_fractions_cells
            return null
        })
        return cb(null,task)
    })
...
/**
 * get_hpms_fractions_one_hour_V2
 * @param {Object} task
 * @param {string} task.ts - The exact hour to fetch
 * @param {Function} cb
 * @returns {}
 * @throws {}
 */
function get_hpms_fractions_one_hour_V2(task,cb){
    // go to couchdb, get fractions for all cells for this hour
...
function get_hpms_scales(task,cb){
    var view = "_design/calvad/_view/grid_yr_day_hour" // incorrectly named!
    var opts = _.assign({}
                       ,task.options.couchdb
                        ,{'view':view
                          ,'include_docs':false
                   ...
                   _.forEach(docs.rows,function(row){
                       ...
                       _.each(aadt_fractions_names,function(k,i){
                           ...
                           if(v){ // null or zero v is an error
                               ...
                               result[geom_id][year][k]=days/v
                           }else{
                               console.log('warning v is undefined for ', row)
                               result[geom_id][year][k]=0
                           }
                           return null
                       })
                       return null
...
exports.get_hpms_fractions_one_hour_V2=get_hpms_fractions_one_hour_V2
