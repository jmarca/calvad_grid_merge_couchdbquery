...
function filter_out_done(task,cb){
    ...
    if(opts.db === undefined) opts.db = opts.state_db
...
var mark_done = function(task,cb){
    var opts = _.assign({}
                       ,task.options.couchdb
                       ,{doc:task.cell_id
                        ,year:task.year
...
var get_hpms_fractions = function(task,cb){
    ...
    var opts = _.assign({}
                       ...
                       ,{startkey:key
                        ...
                        ,'include_docs':true})
    if(opts.db === undefined) opts.db = opts.hpms_db
    getter(opts
          ,function(err,docs){
               if(err) throw new Error(err)
               task.hpms_fractions = {}
               ...
               if(docs !== undefined
                ...
                && docs.rows.length>0){
                   ...
                   if(docs.error !== undefined){
                       throw new Error(console.log(docs))
                   }
               }
...
var get_detector_fractions = function(task,cb){
    ...
    var opts = _.assign({}
                       ...
                       ,{startkey:key
                        ,'endkey':key+'-13' // there is no month 13; gets entire year
                        ,'include_docs':true})
    if(opts.db === undefined) opts.db = opts.grid_merge_couchdbquery_db
    getter(opts
          ,function(err,docs){
               if(err) throw new Error(err)
...
function put_results_doc(task,callback){
    ...
    if(task.options.couchdb.calvad_db===undefined){
        throw new Error('must define options.couchdb.calvad_db in the task object')
    }
    var options = {'cuser':task.options.couchdb.auth.username
                  ,'cpass':task.options.couchdb.auth.password
                  ,'chost':task.options.couchdb.url
                  ,'cport':task.options.couchdb.port
                  ,'cdb'  :task.options.couchdb.calvad_db
                  }
    // make the putter
    var put = putter(options)
    put.overwrite(true)
