 ...
 * @param {Function} cb
 */
function get_scale(grid,year,cb){
    if(hpms_scale[grid] === undefined
       ||hpms_scale[grid][year] === undefined){
        // go fetch
        var q = queue(1)
        q.defer(get_hpms_scales)
        q.awaitAll(function(e,r){
            _.assign(hpms_scale,r)
            return cb(e,hpms_scale[grid][year])
        })
        return null
    }else{
        return cb(null,hpms_scale[grid][year])
    }
}
...
function post_process_couch_query(task,cb){
    ...
    if(task.detector_fractions !== undefined && ! _.isEmpty(task.detector_fractions)){
        ...
        if(task.hpms_fractions === undefined || _.isEmpty(task.hpms_fractions)){
            ...
            get_scale(task.cell_id,task.year,function(e,_scale){
                task.scale = {}
                _.each(scale,function(v,k){
                    if(_scale[k] === undefined){
                        throw new Error ('scale not defined for key ', k)
                    }
                    task.scale[k] = _scale[k]
                })
                return cb(null,task)
            })
        }
...
function post_process_couch_query_one_hour(task,cb){
    ...
    _.forEach(task.hpms_fractions,function(value,cellid){
        ...
        if(task.fractions[cellid] !== undefined) {
            throw new Error('need to check why ',cellid,' is defined in both hpms and detectors for fractions for date ',task.ts,task.year)
        }
        ...
        get_scale(cellid,task.year,function(e,_scale){
            task.scale[cellid] = {}
            _.each(detector_scale,function(v,vehtype){
                if(_scale[vehtype] === undefined){
                    throw new Error ('scale not defined in grid ',cellid,' for key ', vehtype)
                }
                task.scale[cellid][vehtype] = _scale[vehtype]
            })
            return cb(null,task)
        })
    })
