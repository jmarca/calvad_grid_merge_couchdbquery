...
var get_scales = (function (){
    ...
    return function(task,cb){
        ...
        check_if_year_db(_task,function(e,yropts){
            var db = yropts.options.couchdb.db

            if(hpms_scale[db] === undefined){
                // go fetch
                //console.log('fetch from couchdb',db)
                hpms_scale[db]={}
                get_hpms_scales(yropts,function(e,r){
                    _.assign(hpms_scale[db],r)
                    return cb(e,hpms_scale[db][task.year])
                })
                return null
            }else{
                //console.log('have it',db)
                return cb(null,hpms_scale[db][task.year])
            }
        })
        return null
    }
